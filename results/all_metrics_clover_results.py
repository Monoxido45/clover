...
def create_data_list(
    ...
    string_names = [mean_diff, median_diff, max_diff, smis, mean_int, mean_coverage]
    ...
    if path.exists(original_path + folder_path):
        ...
        for i in range(p.shape[0]):
            ...
            new_data = (
                pd.DataFrame(means_array, columns=string_names)
                .assign(p_var=p[i], methods=methods)
                .melt(
                    id_vars=["p_var", "methods"],
    ...
    if path.exists(original_path + folder_path):
        ...
        for i in range(p.shape[0]):
            ...
            correction_folder = (
                original_path
                + folder_path
                + "/{}_score_regression_p_{}_10000_base_model_time".format(kind, p[i])
            )

            # only one string name
            current_data = np.load(
                current_folder + "/" + string + "_p_{}_{}_data.npy".format(p[i], kind)
            )
            ...
            new_data = (
                pd.DataFrame(current_data, columns=methods)
                .assign(p_var=p[i])
                .melt(id_vars=["p_var"], value_vars=methods, var_name="methods")
            )
            ...
            # proportion data
            data_prop = (
                pd.DataFrame(prop_data, columns=methods)
                .assign(p_var=p[i])
                .melt(id_vars=["p_var"], value_vars=methods, var_name="methods")
            )
        ...
        # saving to csv and returning
        data_final.to_csv(
            original_path + folder_path + "/{}_running_times.csv".format(kind)
        )
        time_data.to_csv(
            original_path + folder_path + "/{}_prop_times.csv".format(kind)
        )
        return data_final
...
def create_all_data(
    ...
    for kind in kind_list:
        ...
        if times:
            ...
            data = create_data_times(kind, p=p, other_asym=other_asym).assign(
                data_type=kind
            )

        else:
            # assigning the type of data
            data = create_data_list(kind, p=p, n_it=n_it, other_asym=other_asym).assign(
                data_type=kind
            )
...
def plot_results_by_methods(
    kind,
    p=np.array([1, 3, 5]),
    images_dir="results/metric_figures",
    vars_to_plot=np.array(["mean_diff", "smis"]),
    ...
    if other_asym:
        ...
        figname_others = "performance_other/sim_{}_data_eta_1.5_other_measures".format(
            kind
        )
        figname_p = (
            "performance_vs_p/sim_{}_data_eta_1.5_regression_experiments".format(kind)
        )
    ...
    data_main = (
        pd.read_csv(original_path + folder_path + "/{}_stats.csv".format(kind))
    ...
    g = sns.FacetGrid(
        data_final.query("stats in @vars_to_plot"),
        col="stats",
        col_wrap=2,
        hue="methods",
        hue_order=[
...
if __name__ == "__main__":
    ...
    for kind, kind_name in zip(kinds_list, kinds_names):
        if kind == "asymmetric_V2":
            ...
            data_final_list.append(
                plot_results_by_methods(kind, p=p, other_asym=other_asym).assign(
                    kind=kind_name
                )
            )
            time_data_list.append(
                pd.read_csv(
                    ...
                    + folder_path
                    + "_eta_1.5"
                    + "/{}_prop_times.csv".format(kind)
                ).assign(kind=kind_name)
            )
            abs_time_data_list.append(
                pd.read_csv(
                    original_path
                    + folder_path
                    + "_eta_1.5"
                    + "/{}_running_times.csv".format(kind)
                ).assign(kind=kind_name)
            )
        else:
            folder_path = exp_path + kind + "_data"
            data_final_list.append(
                plot_results_by_methods(kind, p=p).assign(kind=kind_name)
            )
            time_data_list.append(
                pd.read_csv(
                    original_path + folder_path + "/{}_prop_times.csv".format(kind)
    ...
    data_final = pd.concat(data_final_list).query("stats == 'mean_diff'")

    data_marginal = pd.concat(data_final_list).query("stats == 'mean_coverage'")

    # exporting data
    data_show = data_marginal.round(3)
    data_show["value_sd"] = (
        data_show["value"].map(str) + "\n" + "(" + data_show["sd"].map(str) + ")"
    )
    # pivoting methods and smis
    data_show = data_show.pivot(
        index=["kind", "p_var"], columns="methods", values="value_sd"
    )
    ...
    data_final = data_final.assign(
        conformal=lambda df: df["methods"].map(
    ...
    data_count_mean_diff_all = (
        data_final.query("stats == 'mean_diff'")
        .groupby(["p_var", "kind"])
        .apply(lambda df: df.nsmallest(n=1, columns="value", keep="all"))
        .value_counts("methods")
    )

    data_count_mean_diff_conf = (
        data_final.query("stats == 'mean_diff'")
        .query("conformal == True")
        .assign(
            methods=lambda df: df.methods.values.remove_categories(
                ["loforest", "A-loforest", "QRF-TC", "W-loforest"]
            )
        )
        .groupby(["p_var", "kind"])
        .apply(lambda df: df.nsmallest(n=1, columns="value", keep="all"))
    ...
    ax3.bar(
        x=data_count_mean_diff_all.keys(),
        height=data_count_mean_diff_all.values,
        color="tab:blue",
        alpha=0.5,
    )
    ...
    ax1.bar(
        x=data_count_mean_diff_conf.keys(),
        height=data_count_mean_diff_conf.values,
        color="tab:blue",
        alpha=0.5,
    )
    ...
    ax2.bar(
        x=data_count_mean_diff_no_conf.keys(),
        height=data_count_mean_diff_no_conf.values,
        color="tab:blue",
        alpha=0.5,
    )
    ...
    sns.barplot(data=props_time_all, x="kind", y="value", hue="methods", dodge=True)
    plt.xlabel("Data")
