    ...
    for n_var in p:
        ...
        if not (path.exists(original_path + folder_path + var_path)) or completing:
            if not completing:
                ...
                # estimated measures
                smis_vector, wsc_vector = (
                    np.load("smis_p_{}_{}_data.npy".format(n_var, kind)),
                    np.load("wsc_p_{}_{}_data.npy".format(n_var, kind)),
                )

                mean_valid_pred_set, max_valid_pred_set = (
                    np.load("mean_valid_pred_set_p_{}_{}_data.npy".format(n_var, kind)),
                    np.load("max_valid_pred_set_p_{}_{}_data.npy".format(n_var, kind)),
                )

                HSIC_vector, pcor_vector = (
                    np.load("HSIC_p_{}_{}_data.npy".format(n_var, kind)),
                    np.load("pcor_p_{}_{}_data.npy".format(n_var, kind)),
                )
...
def saving_metrics(
    ...
    np.save("wsc_p_{}_{}_data.npy".format(n_var, kind), wsc_vector)
    np.save(
        "mean_valid_pred_set_p_{}_{}_data.npy".format(n_var, kind), mean_valid_pred_set
    )
    np.save(
        "max_valid_pred_set_p_{}_{}_data.npy".format(n_var, kind), max_valid_pred_set
    )
    np.save("pcor_p_{}_{}_data.npy".format(n_var, kind), pcor_vector)
    np.save("HSIC_p_{}_{}_data.npy".format(n_var, kind), HSIC_vector)
