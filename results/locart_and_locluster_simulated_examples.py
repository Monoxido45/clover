...
def compute_conformal_statistics(
    ...
    **kwargs,
    ...
    if "_V2" in kind:
        ...
        kind = "asymmetric"
        folder_path = (
            "/results/pickle_files/locart_experiments_results/{}_data_eta_{}".format(
                kind, eta
            )
        )
    else:
    ...
    for n in n_train:
        ...
        if not (
            path.exists(
                original_path
                + folder_path
                + "/{}_score_{}_dim_{}_{}_samples_measures".format(
                    kind, type_score, d, n
                )
            )
        ):
            print(
                "running the experiments for {} training and calibration samples in the {} setting".format(
                    n, kind
                )
            )
            # measures to be saved at last
            mean_int_length_vector, median_int_length_vector = np.zeros(
                (n_it, 5)
            ), np.zeros((n_it, 5))
            mean_diff_vector, median_diff_vector = np.zeros((n_it, 5)), np.zeros(
                (n_it, 5)
            )
            mean_coverage_vector, median_coverage_vector = np.zeros(
                (n_it, 5)
            ), np.zeros((n_it, 5))
            ...
            for i in range(n_it):
                if (i + 1) % 50 == 0:
                    print(
                        "running {} iteration for {} training samples".format(i + 1, n)
                    )
                ...
                if type_score == "quantile":
                    ...
                    mean_diff_vector[i, 0], median_diff_vector[i, 0] = np.mean(
                        np.abs(cond_locluster_real - (1 - sig))
                    ), np.median(np.abs(cond_locluster_real - (1 - sig)))
                    mean_coverage_vector[i, 0], median_coverage_vector[i, 0] = np.mean(
                        cond_locluster_real
                    ), np.median(cond_locluster_real)
                    (
                        mean_int_length_vector[i, 0],
                        median_int_length_vector[i, 0],
                    ) = np.mean(locluster_interval_len), np.median(
                        locluster_interval_len
                    )
                    ...
                    # several measures
                    mean_diff_vector[i, 1], median_diff_vector[i, 1] = np.mean(
                        np.abs(cond_locart_real - (1 - sig))
                    ), np.median(np.abs(cond_locart_real - (1 - sig)))
                    mean_coverage_vector[i, 1], median_coverage_vector[i, 1] = np.mean(
                        cond_locart_real
                    ), np.median(cond_locart_real)
                    (
                        mean_int_length_vector[i, 1],
                        median_int_length_vector[i, 1],
                    ) = np.mean(locart_interval_len), np.median(locart_interval_len)
                    ...
                    mean_diff_vector[i, 2], median_diff_vector[i, 2] = np.mean(
                        np.abs(uniform_cond_r_real - (1 - sig))
                    ), np.median(np.abs(uniform_cond_r_real - (1 - sig)))
                    mean_coverage_vector[i, 2], median_coverage_vector[i, 2] = np.mean(
                        uniform_cond_r_real
                    ), np.median(uniform_cond_r_real)
                    (
                        mean_int_length_vector[i, 2],
                        median_int_length_vector[i, 2],
                    ) = np.mean(uniform_interval_len), np.median(uniform_interval_len)
                    ...
                    # several measures
                    mean_diff_vector[i, 3], median_diff_vector[i, 3] = np.mean(
                        np.abs(cond_cqr_real - (1 - sig))
                    ), np.median(np.abs(cond_cqr_real - (1 - sig)))
                    mean_coverage_vector[i, 3], median_coverage_vector[i, 3] = np.mean(
                        cond_cqr_real
                    ), np.median(cond_cqr_real)
                    (
                        mean_int_length_vector[i, 3],
                        median_int_length_vector[i, 3],
                    ) = np.mean(cqr_interval_len), np.median(cqr_interval_len)
                    ...
                    # several measures
                    mean_diff_vector[i, 4], median_diff_vector[i, 4] = np.mean(
                        np.abs(cond_qgb_real - (1 - sig))
                    ), np.median(np.abs(cond_qgb_real - (1 - sig)))
                    mean_coverage_vector[i, 4], median_coverage_vector[i, 4] = np.mean(
                        cond_qgb_real
                    ), np.median(cond_qgb_real)
                    (
                        mean_int_length_vector[i, 4],
                        median_int_length_vector[i, 4],
                    ) = np.mean(qgb_interval_len), np.median(qgb_interval_len)
                ...
                if type_score == "regression":
                    ...
                    icp_cond_r_real = real_coverage(
                        icp.predict(X_test, significance=sig), y_mat
                    )
                    icp_interval_len = compute_interval_length(
                        icp.predict(X_test, significance=sig)
                    )

                    # computing icp measures
                    mean_diff_vector[i, 2], median_diff_vector[i, 2] = np.mean(
                        np.abs(icp_cond_r_real - (1 - sig))
                    ), np.median(np.abs(icp_cond_r_real - (1 - sig)))
                    mean_coverage_vector[i, 2], median_coverage_vector[i, 2] = np.mean(
                        icp_cond_r_real
                    ...
                    (
                        mean_int_length_vector[i, 2],
                        median_int_length_vector[i, 2],
                    ) = np.mean(icp_interval_len), np.median(icp_interval_len)
                    ...
                    mean_diff_vector[i, 3], median_diff_vector[i, 3] = np.mean(
                        np.abs(wicp_cond_r_real - (1 - sig))
                    ), np.median(np.abs(wicp_cond_r_real - (1 - sig)))
                    mean_coverage_vector[i, 3], median_coverage_vector[i, 3] = np.mean(
                        wicp_cond_r_real
                    ...
                    (
                        mean_int_length_vector[i, 3],
                        median_int_length_vector[i, 3],
                    ) = np.mean(wicp_interval_len), np.median(wicp_interval_len)
            ...
            # interval length
            np.save(
                "mean_interval_length_n_{}_{}_data.npy".format(n, kind),
                mean_int_length_vector,
            )
            np.save(
                "median_interval_length_n_{}_{}_data.npy".format(n, kind),
                median_int_length_vector,
            )

            # conditional difference
            np.save("mean_diff_n_{}_{}_data.npy".format(n, kind), mean_diff_vector)
            np.save("median_diff_n_{}_{}_data.npy".format(n, kind), median_diff_vector)

            # conditional difference
            np.save(
                "mean_coverage_n_{}_{}_data.npy".format(n, kind), mean_coverage_vector
            )
            np.save(
                "median_coverage_n_{}_{}_data.npy".format(n, kind),
...
if __name__ == "__main__":
    ...
    compute_all_conformal_statistics(type_score="regression", base_model=RandomForestRegressor)
