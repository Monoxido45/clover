...
import time
from lcv.utils import compute_interval_length, split
import gc
...
def compute_metrics(
  ...
  **kwargs):
    ...
    folder_path = "/results/pickle_files/locart_tests/{}_data".format(
        data_name)
    ...
    if not(path.exists(original_path + folder_path + var_path)) or completing:
      if not completing:
        ...
        mean_int_length_vector, mean_coverage_vector = np.zeros(n_it), np.zeros(n_it)
        init_it = 0
      else:
      ...
      for it in range(init_it, n_it):
        ...
        mean_coverage_vector[it] = np.mean(
            marg_cover
        )
        ...
        if (it + 1) % 25 == 0 or (it + 1 == 1) or save_all:
          ...
          mean_int_length_vector, mean_coverage_vector)
    ...
    print("Time Elapsed to compute metrics in the {} data: {}".format(data_name, end_kind))
    return end_kind
...
if __name__ == '__main__':
  ...
  if model == "Random Forest":
    ...
    for data in data_names:
      exp_time = compute_metrics(data_name = data, 
      base_model = RandomForestRegressor, random_state = random_state)
      print("Time elapsed to conduct testing: {}".format(exp_time))
      
      np.save("results/pickle_files/locart_tests/{}_running_time.npy".format(data), 
      np.array([exp_time]))
  ...
  for data in data_names:
    ...
    marginal_coverage = np.load(original_path + folder_path + "/mean_coverage_{}_data.npy".format(data))
    marginal_covers.append(np.mean(marginal_coverage))
    ses.append(2*np.std(marginal_coverage))
  
  print(pd.DataFrame({"data" : data_names, "marginal_cover" : marginal_covers, "2*se" : ses}))
