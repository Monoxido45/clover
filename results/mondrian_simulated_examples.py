...
from clover.scores import LocalRegressionScore
from clover.locart import MondrianRegressionSplit
...
def add_mondrian_conformal_statistics(
    kind="homoscedastic",
    n_it=200,
    ...
    for n in n_train:
        ...
        if not (
            path.exists(
                original_path
                + folder_path
                + "/{}_score_{}_dim_{}_{}_samples_measures/mondrian".format(
                    kind, type_score, d, n
                )
            )
        ):
            print(
                "running the mondrian experiments for {} training and calibration samples in the {} setting".format(
                    n, kind
                )
            )
            # measures to be saved at last
            mean_int_length_vector, median_int_length_vector = np.zeros(n_it), np.zeros(
                n_it
            )
            mean_diff_vector, median_diff_vector = np.zeros(n_it), np.zeros(n_it)
            mean_coverage_vector, median_coverage_vector = np.zeros(n_it), np.zeros(
                n_it
            )
            ...
            for i in range(n_it):
                ...
                mean_diff_vector[i], median_diff_vector[i] = np.mean(
                    np.abs(micp_cond_r_real - (1 - sig))
                ), np.median(np.abs(micp_cond_r_real - (1 - sig)))
                mean_coverage_vector[i], median_coverage_vector[i] = np.mean(
                    micp_cond_r_real
                ), np.median(micp_cond_r_real)
                mean_int_length_vector[i], median_int_length_vector[i] = np.mean(
                    micp_interval_len
                ), np.median(micp_interval_len)
...
def compute_all_mondrian_conformal_statistics(
    ...
    for kinds in kind_lists:
        times_list.append(
            add_mondrian_conformal_statistics(
                kind=kinds, n_it=n_it, n_train=n_train, d=d
            )
        )
    end_exp = time.time() - start_exp
    print("Time elapsed to conduct all experiments: {}".format(end_exp))
    np.save(
        "results/pickle_files/locart_experiments_results/mondrian_running_times.npy",
...
if __name__ == "__main__":
    print(
        "We will now compute all conformal statistics for mondrian split for several simulated examples"
    )
