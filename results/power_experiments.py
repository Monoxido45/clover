...
from clover.valid_pred_sets import LinearQuantileRegression
from clover.valid_pred_sets import GradientBoostingQuantileRegression
import numpy as np
import os
from os import path
import time
...
def compute_power(
    ...
    npower=100,
    B=350,
    ntrain=np.arange(1000, 25000, 5000),
    ...
    print(
        "running power experiments for {} regression and {} data".format(type_reg, kind)
    )
    for i in range(ntrain.shape[0]):
        power = np.zeros(1)
        if not (
            path.exists(
                "results/pickle_files/power_tests_{}_n_{}_d_{}_{}_{}_data.npy".format(
                    type_reg, ntrain[i], d, coverage_evaluator, kind
                )
            )
        ):
            print("running {} experiment with {} samples".format(i + 1, ntrain[i]))
            seeds_calib, seeds_train = np.random.randint(
                1e8, size=npower
            ), np.random.randint(1e8, size=npower)
            seeds_gbqr, seeds_split = np.random.randint(
                1e8, size=npower
            ), np.random.randint(1e8, size=npower)
            ...
            for j in range(npower):
                ...
                if type_reg == "gb_quantile":
                    gbqr = GradientBoostingQuantileRegression(
                        n_estimators=200, random_state=seeds_gbqr[j]
                    )
                    gbqr.fit(sim_obj.X, sim_obj.y)
                    hyp_gbqr_quant = Valid_pred_sets(
                        gbqr, sig, coverage_evaluator=coverage_evaluator
                    )
                    ...
                    p_values[j] = hyp_gbqr_quant.monte_carlo_test(
                        B=B, random_seed=random_seed, par=par
                    )["p-value"]
                    print(p_values[j])
                ...
                # using linear quantile regression
                    ...
                    hyp_lqr_quant = Valid_pred_sets(
                        lqr, sig, coverage_evaluator=coverage_evaluator
                    )
                    # generating calibration
                    sim_kind(ntrain[i], random_seed=seeds_train[j])
                    hyp_lqr_quant.fit(sim_obj.X, sim_obj.y, random_seed=seeds_split[j])
                    p_values[j] = hyp_lqr_quant.monte_carlo_test(
                        B=B, random_seed=random_seed, par=par
                    )["p-value"]
