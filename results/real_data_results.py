...
def create_data_list(
    data_name, n_it=100, exp_path="/results/pickle_files/real_data_experiments/"
):
    ...
    mean_int, mean_int_cover, mean_coverage = (
        "mean_interval_length",
        "mean_interval_length_cover",
        "mean_coverage",
    )
    smis = "smis"
    ...
    string_names = [smis, mean_int, mean_int_cover, mean_coverage]
    ...
    if path.exists(original_path + folder_path):
        ...
        data_final = (
            ...
            .melt(id_vars=["methods"], value_vars=string_names, var_name="stats")
            .assign(sd=sd_array / np.sqrt(n_it))
        )

        # saving to csv and returning
        data_final.to_csv(
            original_path + folder_path + "/{}_stats.csv".format(data_name)
        )
...
def create_data_times(
    data_name, exp_path="/results/pickle_files/real_data_experiments/"
):
    # folder path
    folder_path = exp_path + data_name + "_data"
    print(folder_path)
    ...
    if path.exists(original_path + folder_path):
        ...
        # proportion data
        data_prop = pd.DataFrame(prop_data, columns=methods).melt(
            value_vars=methods, var_name="methods"
        )

        # melted data
        data_final = pd.DataFrame(current_data, columns=methods).melt(
            value_vars=methods, var_name="methods"
        )

        # saving to csv and returning
        data_final.to_csv(
            original_path + folder_path + "/{}_running_times.csv".format(data_name)
        )
        data_prop.to_csv(
            original_path + folder_path + "/{}_prop_times.csv".format(data_name)
        )
        return data_final
...
def plot_results(
    ...
    for name in data_names_list:
        ...
        current_data = (
            pd.read_csv(original_path + folder_path + "/{}_stats.csv".format(name))
        ...
        time_data = pd.read_csv(
            original_path + folder_path + "/{}_prop_times.csv".format(name)
        ).assign(data=name)
        time_data_list.append(time_data)

        original_time_data = pd.read_csv(
            original_path + folder_path + "/{}_running_times.csv".format(name)
        ).assign(data=name)
    ...
    print(
        original_time_data.query("data in @data_w").groupby(["data", "methods"]).mean()
    )
    ...
    ax2.bar(
        x=data_count_smis_no_conf.keys(),
    ...
    sns.barplot(data=time_data, x="data", y="value", hue="methods", dodge=True)
...
smis_data["value_sd"] = (
    smis_data["value"].map(str) + "\n (" + smis_data["sd"].map(str) + ")"
)
# pivoting methods and smis
a = smis_data.pivot(index="data", columns="methods", values="value_sd")
