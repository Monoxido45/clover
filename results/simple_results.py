...
from lcv.utils import compute_interval_length, HSIC_correlation, pearson_correlation, ILS_coverage, split, smis
...
**kwargs):
    ...
    print("Number of samples used for fitting coverage evaluator: {}".format((1 - valid_test_size)*test_size*X.shape[0]))
    print("Number of samples used to compute conditional coverage difference {}".format(valid_test_size*test_size*X.shape[0]))
    ...
    if type_score == "regression":
        ...
        micp = MondrianRegressionSplit(model, alpha = sig, is_fitted = is_fitted, k = nbins, **kwargs)
        ...
        # valid pred sets
        vp = Valid_pred_sets(conf = micp, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        start_loc = time.time()
        # fitting locart
        locart_obj = LocartSplit(cart_type = "CART", nc_score = RegressionScore, base_model = model,
       ...
       # predictions
        ...
        vp = Valid_pred_sets(conf = locart_obj, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        vp = Valid_pred_sets(conf = acpi, alpha = sig, islcp = True, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        start_loc = time.time()
        # fitting locart
        wlocart_obj = LocartSplit(cart_type = "RF", nc_score = LocalRegressionScore, base_model = model,
                                  is_fitted = is_fitted, alpha = sig, split_calib = split_calib, **kwargs)
       ...
       # predictions
        ...
        vp = Valid_pred_sets(conf = wlocart_obj, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        vp = Valid_pred_sets(conf = dlocart_obj, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        start_loc = time.time()
        # fitting locart
        rf_locart_obj = LocartSplit(cart_type = "RF", nc_score = RegressionScore, base_model = model, 
                                    is_fitted = is_fitted, alpha = sig, split_calib = split_calib, **kwargs)
       ...
       # predictions
        ...
        vp = Valid_pred_sets(conf = rf_locart_obj, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        start_loc = time.time()
        # fitting locart
        rf_dlocart_obj = LocartSplit(cart_type = "RF", nc_score = RegressionScore, base_model = model, 
       ...
       # predictions
        ...
        vp = Valid_pred_sets(conf = rf_dlocart_obj, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        vp = Valid_pred_sets(conf = icp, alpha = sig, coverage_evaluator = "CART", isnc = True, prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
       ...
       # marginal coverage
        ...
        vp = Valid_pred_sets(conf = wicp, alpha = sig, coverage_evaluator = "CART", prune = valid_prune, split_train = valid_split)
        vp.fit(data["X_test"], data["y_test"], test_size = valid_test_size, min_samples_leaf = valid_sample_leaf)
        ...
        all_results =  pd.DataFrame(data = {"Methods":["LOCART", "DLOCART", "RF-LOCART", "RF-DLOCART", "LCP-RF", "Weighted LOCART", "Regresion split", 
                                                       "Weighted regression split", "Mondrian regression split"],
        "Pearson correlation":[locart_pcor, dlocart_pcor, rf_locart_pcor, rf_dlocart_pcor, acpi_pcor, wlocart_pcor,  icp_pcor, wicp_pcor, micp_pcor],
        "HSIC correlation":[locart_hsic, dlocart_hsic, rf_locart_hsic, rf_dlocart_hsic, acpi_hsic, wlocart_hsic, icp_hsic, wicp_hsic, micp_hsic],
        "mean pred dif":[pred_set_dif_locart, pred_set_dif_dlocart, pred_set_dif_rf_locart, pred_set_dif_rf_dlocart, pred_set_dif_acpi, 
                         pred_set_dif_wlocart, pred_set_dif_icp, pred_set_dif_wicp, pred_set_dif_micp],
        "max pred dif":[max_set_dif_locart, max_set_dif_dlocart, max_set_dif_rf_locart, max_set_dif_rf_dlocart, max_set_dif_acpi,  
                        max_set_dif_wlocart, max_set_dif_icp, max_set_dif_wicp, max_set_dif_micp],
        "smis":[locart_smis, dlocart_smis, rf_locart_smis, rf_dlocart_smis, acpi_smis, wlocart_smis, icp_smis, wicp_smis, micp_smis],
        "Average marginal coverage":[locart_ave_marginal_cov, dlocart_ave_marginal_cov, rf_locart_ave_marginal_cov, rf_dlocart_ave_marginal_cov, 
                                     acpi_ave_marginal_cov, wlocart_ave_marginal_cov, icp_ave_marginal_cov, wicp_ave_marginal_cov, micp_ave_marginal_cov],
        "Average interval length":[locart_interval_len, dlocart_interval_len, rf_locart_interval_len, rf_dlocart_interval_len, acpi_interval_len,
                                    wlocart_interval_len, icp_interval_len, wicp_interval_len, micp_interval_len],
        "Average interval length given coverage":[locart_interval_len_cover, dlocart_interval_len_cover, rf_locart_interval_len_cover, 
                                                  rf_dlocart_interval_len_cover, acpi_interval_len_cover, 
                                                  wlocart_interval_len_cover, icp_interval_len_cover, wicp_interval_len_cover, micp_interval_len_cover]})
    return all_results
...
if __name__ == '__main__':
  ...
  for data_name in data_names:
    ...
    stats.to_csv(original_path + folder_path + "/exp_stats_{}.csv".format(data_name))
