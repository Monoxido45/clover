...
def testing_metrics_sim(
    ...
    **kwargs):
    ...
    start_loc = time.time()
    locart_obj = LocartSplit(nc_score = RegressionScore, cart_type = "CART", base_model = model, 
    alpha = sig, split_calib = split_calib, is_fitted = True)
    ...
    start_loc = time.time()
    rf_locart_obj = LocartSplit(nc_score = RegressionScore, cart_type = "RF", base_model = model, alpha = sig, 
    split_calib = split_calib, is_fitted = True, **kwargs)
    ...
    start_loc = time.time()
    dlocart_obj = LocartSplit(nc_score = RegressionScore, cart_type = "CART", base_model = model, alpha = sig, 
    split_calib = split_calib, is_fitted = True, weighting = True,**kwargs)
    ...
    alpha = sig, split_calib = split_calib, is_fitted = True, weighting = True, **kwargs)
    ...
    wlocart_obj = LocartSplit(nc_score = LocalRegressionScore, cart_type = "RF", 
    base_model = model, alpha = sig, split_calib = split_calib, is_fitted = True, **kwargs)
...
if __name__ == '__main__':
  ...
  for kind in kinds_list:
    for p in [1,3,5]:
      ...
      stats = testing_metrics_sim(kind = kind, signif_vars = p, noise = noise, random_state = 650)
      stats.to_csv(original_path + folder_path + "/exp_stats_{}_nvar_{}.csv".format(kind, p))
